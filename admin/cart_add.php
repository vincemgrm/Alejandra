<?php
 ...
 $stmt2 = $conn->prepare("SELECT COUNT(*) AS numrows FROM transaction");
                      ...
                      $ctr = sprintf("%05d",$ctr);
		$stmt = $conn->prepare("SELECT *, COUNT(*) AS numrows FROM cart WHERE product_id=:id and user_id=:user_id and transaction_no=:transaction_no");
		$stmt->execute(['id'=>$product, 'user_id'=>$id, 'transaction_no'=>$ctr]);
		$row = $stmt->fetch();
